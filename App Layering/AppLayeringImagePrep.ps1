<#
####################################################################
#    File Name     : AppLayeringImagePrep.ps1                      #
#    Author        : George Spiers with thanks to Trentent Tye for #
#                    allowing me to include his Ghost Device       #
#                    removal script.                               #
#    Email         : george@jgspiers.com                           #
#    Twitter       : @JGSpiers                                     #
#    Website       : www.jgspiers.com                              #
#    Date Created  : 18.08.2017                                    #
#    Tested on     : Windows Server 2016 1607,                     #
#                    Windows Server 2012 R2,                       #
#                    Windows Server 2008 R2,                       #
#                    Windows 10 1607,                              #
#                    Windows 7.                                    #
#    Description   : This scripts runs a number of tasks to clean  #
#                    out temporary folders, clear out Event Log    #
#                    entries, remove ghost devices from Device     #
#                    Manager, and run NGEN native image            #
#                    regenerations before shutting down the        #   
#                    image in preparation for layer finalisation.  #
#                    If running this script on Windows 8.1+ or     #
#                    Windows Server 2012 R2+ there is an extra     #
#                    firewall check that looks to see if any of    #
#                    the firewall profiles are disabled and if so, #
#                    asks you if you want to enable them. When     #
#                    patching withn an OS Layer version my         #
#                    patching software requires a number of ports  #
#                    to be open and it is easier just to           #
#                    temporarily disable the firewall until        #
#                    patching is done. The problem was remembering #
#                    to enable the firewall again!                 #                                           
#    Note          : You should run this script each time you      #
#                    create an OS Layer/Layer version, Application #
#                    Layer/Layer version and Platform Layer/Layer  #
#                    version.                                      #
#    Instructions  : You must run this script as an administrator. #
#                    You must also set the PowerShell Execution    #
#                    Policy to atleast "RemoteSigned" or           #
#                    "Unrestricted".                               #
#    Change Log    : 23.08.2017: Removed Read-Host that asked for  #
#                                script path.                      #
#                    29.09.2018: Added disable windows update      #
#                                service.                          #
#                                Added disable IPv6 if PVS         #
#                                Target Device software is         #
#                                installed                         #
####################################################################
#>
...
$FirewallStates = Get-NetFirewallProfile | Select-Object Name, Enabled | where {$_.Enabled -eq "False"}
foreach ($FirewallState in $FirewallStates) {
    $Question = Read-Host The ($FirewallState.Name) Firewall is disabled. Do you want to enable it? Y/N 
         while ("Y","N" -notcontains $Question) 
            {$Question = Read-Host "Enter Y or N"} 
    if ($Question -eq "Y") 
            { Set-NetFirewallProfile -Name ($FirewallState.Name) -Enabled True} 
                                            }
                       }
if ($Present = Get-Command Get-NetFirewallProfile -ErrorAction SilentlyContinue) { CheckFirewall }
...
Write-Host Removing ghost devices from machine. -ForegroundColor Green
...
Write-Host Stop Windows Update service, if running -ForegroundColor Green
...
Write-Host Disable Windows Update service, if enabled -ForegroundColor Green
If (((Get-Service -Name "wuauserv").StartType -eq "Automatic" -or (Get-Service -Name "wuauserv").StartType -eq "Manual"))
{
    Set-Service -Name "wuauserv" -StartupType "Disabled"
}
    else
    {
        Write-Host Windows Update service is disabled
    }
